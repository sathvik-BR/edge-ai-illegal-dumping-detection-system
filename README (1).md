...
An edge-AI surveillance system that detects illegal dumping in real time using a fine-tuned YOLOv8 model, a HOLDING → RELEASED → DEPARTED evidence-based state machine, Telegram alerts, and a live Flask dashboard. Supports multiple simultaneous camera sources (laptop webcam + phone via IP Webcam).
...
Model weights are not stored in this repo (kept out via `.gitignore` — binary files bloat git history). Download `best.pt` from: **[https://drive.google.com/file/d/1nDwYC6pj08wDdcYP5WxFsSVfMpvBS7aq/view?usp=sharing]**
...
**Start detection (in a separate terminal), for your laptop webcam:**
```bash
# Windows PowerShell
